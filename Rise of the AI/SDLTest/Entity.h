...
enum EntityType { PLATFORM, PLAYER, ENEMY, TEXT  };
enum AIType     { ZOOM, GUARD, MONKEY};
...
private:
    ...
    int m_walking[6][4]; // 6x4 array for walking animations
...
public:
    ...
    // ————— METHODS ————— //
    Entity();
    Entity(GLuint texture_id, float speed, glm::vec3 acceleration, float jump_power, int walking[6][4], float animation_time,
	...
	int      const get_m_walking() const { return m_walking[6][4]; }
    ...
    void draw_text(ShaderProgram* program, std::string text,
        float font_size, float spacing, glm::vec3 position);
    // Setter for m_walking
    void set_walking(int walking[6][4])
    {
        for (int i = 0; i < 6; ++i)
        {
            for (int j = 0; j < 4; ++j)
